export default function UserListsLoading() {
  return (
    <>
      <tr className="bg-[#f3f6ff]">
        <td data-cell="Name:" className="flex items-center gap-2 px-4 py-2">
          <div className="h-8 w-8 rounded-full bg-[#dbe4ff]"></div>
          <div className="h-5 w-28 rounded-md bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Email:" className="px-4 py-2">
          <div className="h-4 w-36 rounded-md bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Role:" className="px-4 py-2">
          <div className="h-8 w-24 rounded-full bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Date added:" className="px-4 py-2">
          <div className="h-5 w-24 rounded-md bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Verified:" className="px-4 py-2">
          <div className="h-5 w-5 rounded-full bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="" className="relative px-4 py-2">
          <div className="absolute -top-[12rem] right-2 md:right-4 md:top-4">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              className="size-6 stroke-[#dbe4ff]"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M12 6.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 12.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 18.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5Z"
              />
            </svg>
          </div>
        </td>
      </tr>
      <tr className="bg-[#f3f6ff]">
        <td data-cell="Name:" className="flex items-center gap-2 px-4 py-2">
          <div className="h-8 w-8 rounded-full bg-[#dbe4ff]"></div>
          <div className="h-5 w-28 rounded-md bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Email:" className="px-4 py-2">
          <div className="h-4 w-36 rounded-md bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Role:" className="px-4 py-2">
          <div className="h-8 w-24 rounded-full bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Date added:" className="px-4 py-2">
          <div className="h-5 w-24 rounded-md bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Verified:" className="px-4 py-2">
          <div className="h-5 w-5 rounded-full bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="" className="relative px-4 py-2">
          <div className="absolute -top-[12rem] right-2 md:right-4 md:top-4">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              className="size-6 stroke-[#dbe4ff]"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M12 6.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 12.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 18.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5Z"
              />
            </svg>
          </div>
        </td>
      </tr>
      <tr className="bg-[#f3f6ff]">
        <td data-cell="Name:" className="flex items-center gap-2 px-4 py-2">
          <div className="h-8 w-8 rounded-full bg-[#dbe4ff]"></div>
          <div className="h-5 w-28 rounded-md bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Email:" className="px-4 py-2">
          <div className="h-4 w-36 rounded-md bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Role:" className="px-4 py-2">
          <div className="h-8 w-24 rounded-full bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Date added:" className="px-4 py-2">
          <div className="h-5 w-24 rounded-md bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Verified:" className="px-4 py-2">
          <div className="h-5 w-5 rounded-full bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="" className="relative px-4 py-2">
          <div className="absolute -top-[12rem] right-2 md:right-4 md:top-4">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              className="size-6 stroke-[#dbe4ff]"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M12 6.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 12.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 18.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5Z"
              />
            </svg>
          </div>
        </td>
      </tr>
      <tr className="bg-[#f3f6ff]">
        <td data-cell="Name:" className="flex items-center gap-2 px-4 py-2">
          <div className="h-8 w-8 rounded-full bg-[#dbe4ff]"></div>
          <div className="h-5 w-28 rounded-md bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Email:" className="px-4 py-2">
          <div className="h-4 w-36 rounded-md bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Role:" className="px-4 py-2">
          <div className="h-8 w-24 rounded-full bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Date added:" className="px-4 py-2">
          <div className="h-5 w-24 rounded-md bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="Verified:" className="px-4 py-2">
          <div className="h-5 w-5 rounded-full bg-[#dbe4ff]"></div>
        </td>
        <td data-cell="" className="relative px-4 py-2">
          <div className="absolute -top-[12rem] right-2 md:right-4 md:top-4">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              className="size-6 stroke-[#dbe4ff]"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M12 6.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 12.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 18.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5Z"
              />
            </svg>
          </div>
        </td>
      </tr>
    </>
  );
}
